...
impl Stream for ClientStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match futures::ready!(Pin::new(&mut self.as_mut().stream).poll_next(cx)) {
            Some(Ok(msg)) => {
...
pub struct Outgoing {
    ...
    rx_outgoing: UnboundedReceiver<Message>,
    pending: usize,
    ready: bool,
}
...
impl Future for Outgoing {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        loop {
            ...
            while self.as_ref().pending > 0 {
                match Pin::new(&mut self.sink).poll_flush(cx) {
                    Poll::Ready(Ok(())) => self.as_mut().pending = 0,
            ...
            if !self.as_ref().ready {
                futures::ready!(Pin::new(&mut self.sink).poll_ready(cx))?;
                self.as_mut().ready = true;
            }

            let message = match futures::ready!(Pin::new(&mut self.rx_outgoing).poll_next(cx)) {
                Some(message) => message,
                None => panic!("rx receiver ended unexpectedly"),
            };

            Pin::new(&mut self.sink).start_send(message)?;
            self.as_mut().pending += 1;
            self.as_mut().ready = false;
        }
    }
}
